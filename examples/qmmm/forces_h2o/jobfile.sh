#!/bin/bash
#SBATCH --time=0-12:1:00
#SBATCH --partition=price,main
#SBATCH --job-name=qmmm_forces
##SBATCH --nodelist=gpun003
#SBATCH --nodes=1                   # Number of nodes you require
#SBATCH --ntasks=3                # Total # of tasks across all nodes
#SBATCH --mem=100G                   # Real memory (RAM) required          
#SBATCH --output=slurm.%N.%j.out     # STDOUT output file
#SBATCH --error=slurm.%N.%j.err      # STDERR output file (optional)
#SBATCH --export=ALL                 # Export you current env to the job env
#SBATCH --cluster=amareln

env=edftpy_qmmm_update
software=new_software
mbx_type=mbx-dipoles
#mbx_type=mbx-python-old
#mbx_type=mbx-non-dipoles

#env=edftpy_qmmm
#software=software

module load python/3.8.5-gc563 intel/19.1.1 gsl/2.5-bd387 libffi/3.3-gc563
source /projectsn/mp1009_1/motas/3_QMMM/$software/$env/bin/activate
export LD_PRELOAD=/opt/sw/packages/intel/19.1.3/mkl/lib/intel64/libmkl_rt.so
export LD_LIBRARY_PATH="/opt/sw/packages/intel/19.1.3/mkl/lib/intel64/:$LD_LIBRARY_PATH"
export LDFLAGS="-L//opt/sw/packages/intel/19.1.3/mkl/lib/intel64"
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/projectsn/mp1009_1/motas/3_QMMM/$software/q-e/external/devxlib/src
export qedir="/projectsn/mp1009_1/motas/3_QMMM/$software/q-e"
export MBX_HOME=/projectsn/mp1009_1/motas/3_QMMM/$software/$mbx_type/
export PYTHONPATH=$PYTHONPATH:${MBX_HOME}/plugins/python/mbx
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/projectsn/mp1009_1/motas/3_QMMM/$software/dftd3/lib64/

NODES=$SLURM_JOB_NUM_NODES
NTASKS=$SLURM_NTASKS
ulimit -n 4096

#for ifile in ../../xxyz06--2.xyz ../../xxyz06--1.xyz ../../xxyz06.xyz ../../xxyz06-+1.xyz ../../xxyz06-+2.xyz ;
for ifile in xxyz06--2.xyz xxyz06--1.xyz xxyz06.xyz xxyz06-+1.xyz xxyz06-+2.xyz ;
#for ifile in xxyz06--2.xyz ;
do
  echo "$ifile"
  #mv $ifile $ifile'.xyz'
  sed "19c cell-file = $ifile"  mbx-tmp.ini > mbx.ini
#  srun -N $NODES -n $NTASKS --mpi=pmi2 python3 -m cProfile -o prof.out -m edftpy --run mbx.ini --mpi4py 2>&1 | tee $ifile.out
  srun -N $NODES -n $NTASKS --mpi=pmi2 python3 -m edftpy --run mbx.ini --mpi4py  |tee $ifile.out
  cp  sub_ks1.out   $ifile'_ks1.rec' 
  cp  sub_mm.out   $ifile'_mm.rec' 
done

