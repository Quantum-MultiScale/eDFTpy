...
#SBATCH --job-name=pd
#SBATCH --nodelist=haln074,haln104,haln082,haln111
#SBATCH --nodes=4
#SBATCH --ntasks=160                # Total # of tasks across all nodes
...
do
	echo "$ifile"
	srun -N $NODES -n $NTASKS --mpi=pmi2 python3 -m edftpy --run qmmm.ini --mpi4py  |tee $ifile.out
done
